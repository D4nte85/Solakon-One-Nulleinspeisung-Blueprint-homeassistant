...
flowchart TD
    ...
    ZONE_CHECK -- "CASE D   Cycle = on OR Charge-Bool = on   AND Mode ∉ {'1','3'} ← '3' explicitly excluded!   AND (Charge-Bool = on OR SOC > Zone 3 threshold)" --> RECOVERY
    ...
    subgraph SG_PI ["🧠 PI Controller Operation"]
        ...
        CALC_AC["⚡ AC Charging — PI Script (ac_charge_mode=true)   raw_error = (ac_charge_offset − grid) × error_share   error_share (Pool 2, AC charging): missing_i / Σ(missing_j) — only among instances currently AC charging at the same time, set by power distribution (default 1.0)   independent of Pool 1 (Zero-Export) — prevents error_share=0 freeze when instance isn't in mode '1'   missing_i = (charge target_i−SOC_i)/100 × Cap_i   (Cap_i = Cap sensor kWh or 100 if not set)   max_power = ac_charge_power_limit   Capacity clamping   correction = P·error + I·integral_candidate   (separate ac_charge_p/i_factor)   new_power = current + correction   clamp(0, charge limit) → Output → inverter   Anti-windup: integral = (final − current − P·error) / I → clamp(±effective_max)   Wait time   (at_max / at_min guards NOT applied)"]
        ...
        STALL_GATE{{"Output stalled?   At-max-limit AND no AC charging   AND |actual − dynamic_max| > 5 % · dynamic_max   AND actual sensor unchanged for > 300 s (last_updated)   AND mode unchanged for > 300 s (last_changed)"}}
        STALL_GATE -- No --> END_OK
        STALL_GATE -- Yes --> STALL_RECOVER
        STALL_RECOVER["⚠️ Stall — recovery   Integral = 0   Output → 0 W   Timer toggle (3598↔3599)   Mode → '0'   → next run: case D (timer toggle + mode '1'), PI ramps up"]
