...
### Behoben
- Fall-H-Austritt (AC Laden Ende): Vergleich `eigener Output ≤ 0 W` (aus V309) auf `|eigener Output| ≤ Toleranz` geändert. Der einseitige `≤ 0`-Vergleich sollte isolierte Ein-Sekunden-Nullwerte durch Modbus-Rauschen abfangen, machte den Guard aber empfindlicher als vorher: jeder noch so kleine negative Ausreißer (z. B. `-0,1 W`) erfüllt die Bedingung jetzt ebenfalls, während der vorherige exakte `== 0`-Vergleich solche Ausreißer nie getroffen hätte. Bei Setups mit häufigem AC-Laden (insbesondere reine AC-Anbindung ohne PV) beobachtet als periodischer Moduswechsel 3↔1 alle ~15–20 s während aktivem AC-Laden, mit hartem Output-Reset auf 0 W bei jedem Fehlauslöser (Discussion #58, Feldbeleg 27.08.). Fix: symmetrisches Toleranzband um 0 W (`|Output| ≤ tolerance`, dieselbe Einstellung wie die PI-Konvergenzprüfung) statt einseitigem Vergleich — fängt Rauschen in beide Richtungen ab, ohne bei jedem winzigen negativen Ausreißer während echtem Weiterladen auszulösen
