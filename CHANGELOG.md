...
- ⚠️ **Breaking:** `PI-Controller.yaml`: removed the unused `tolerance` field, the main blueprint no longer passes it. Tolerance remains a deadband in front of the script call. **Re-import the PI script together with the main blueprint** — an old script fails with the V310 blueprint with `UndefinedError: 'tolerance' is undefined` (DE-repo Issue #86). A new script with an older blueprint keeps working
- ⚠️ **Breaking:** PV Forecast Tariff Suppression now expects a **daily yield in kWh** instead of a power in W. Previously the raw sensor value was compared against a W threshold (default 5000) without any unit detection — but tariff charging runs at night, where any power forecast sits at 0, so the suppression never triggered; the description itself already named Solcast `energy_production_today`, an energy sensor. New variable `pv_forecast_energy_float` normalizes Wh ÷1000 and MWh ×1000, threshold now 0–50 kWh with a default of 15. Review sensor choice and threshold after updating. Surplus Forecast Override and the exit lock deliberately stay on power in W — what matters there is the curtailment risk right now; the input texts now separate the two explicitly
...
### Fixed
- `solakon_power_distribution.yaml`: capacity sensors were read without a unit check, a Wh sensor weighed 1000-fold, and an unavailable instance alone got the fallback 100 next to real kWh from the others. Both skewed the weights and thus `error_share` in every instance's PI. Wh is now converted to kWh; if a value is missing for an active instance, all use 100 with a logbook warning
- Cases B and C checked `soc < soc_conservation_limit`, while Case E and the zone table assign the threshold itself to Zone 3. With SOC exactly at the threshold no case applied and discharging ran one point too far. Both now check `<=`
- Case D did not recover a dropped charging session: it required cycle = `on` and SOC > Zone 3 threshold, neither of which holds during AC or tariff charging in Zone 2. Case D now also applies while a charging session is active, dropping the SOC condition in that case
- PV forecast suppression only affected GT and TM, not `price_discharge_locked`: on sunny days TM no longer stopped discharging, but Cases A and E could not start it either. The discharge lock now lifts while suppression is active
- `inverter_entity_max` and `entity_max` in the PI controller read the power entity's `max` attribute, which the Modbus integration sets to the register limit (100000) rather than the device limit. Both are now capped at 1200 W, the `9999` fallback is gone
- Regulation could fall permanently silent in the saturated state: when the setpoint sits at the upper limit and the grid error persists in the same direction, `at_max_limit` stays satisfied and the PI is never called — the `default:` branch only decays the integral. An inverter stalled in exactly that state, without changing its mode register, never receives another command; case D does not apply either, because it requires a mode outside `'1'`/`'3'`. Since no write was attempted, nothing could fail and be reported. A new branch in the `default:` block checks actual power against the limit: more than 5 % deviation while the actual sensor's `last_updated` has been standing still for over 300 s sets integral = 0, output → 0 W, timer toggle and mode → `'0'` — on the next run case D brings the device back. Repeats at most every 300 s (mode entity's `last_changed`). Normal discharge mode only; Zone 0, tariff charging and AC charging deliberately hold the output below their limit. Ported from integration v2.3.2, field report there: 19 h of 0 W output during an active discharge cycle, recovery only after a hardware restart of the inverter (integration issue [#32](https://github.com/D4nte85/Solakon-One-Nulleinspeisung-integration/issues/32))
- Case H exit (AC Charging End): comparison `Output ≤ 0 W` (from V309) changed to `|Output| ≤ Tolerance`. Field report (2026-08-27, pure AC connection without PV) analyzed via history chart: the sensor configured as `active_power_entity` follows the same sign convention as the grid sensor (positive = import, negative = feed-in/surplus consumption) — during active AC charging it stays clearly negative throughout (down to −400 W in the field report), not just near 0. The one-sided `≤ 0` comparison was therefore satisfied for practically the entire charging session regardless of actual charge power — effectively only the grid condition remained. Since charging itself reduces the measured grid surplus, regulation regularly reaches that threshold as a direct consequence of its own work — case H then fired immediately even during substantial ongoing charging (field report: reset at ~300 W commanded power), followed by an immediate re-entry via case G since the grid deficit persists after the reset — a periodic mode switch 3↔1 every ~15–20 s with a hard output reset to 0 W (related to, but a distinct cause from, DE-repo Discussion #58). Fix: `|Output| ≤ tolerance` (the same setting used by the PI convergence check) now requires charge power to have genuinely wound down near zero before the grid branch applies — an ongoing charge session with meaningful power no longer satisfies the condition
